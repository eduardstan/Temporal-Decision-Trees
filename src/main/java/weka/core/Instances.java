...
RevisionHandler {
  ...
  /**
   * Adds one instance to the end of the set. Shallow copies instance if the flag is true,
   * else it simply adds it. Increases the size of the dataset if it is not large enough.
   * Does not check if the instance is compatible with the dataset. Note: String or
   * relational values are not transferred.
   *
   * @param instance the instance to be added
   * @param shallowCopy flag for shallow copying or not
   */
  public boolean add(/* @non_null@ */Instance instance, boolean shallowCopy) {
