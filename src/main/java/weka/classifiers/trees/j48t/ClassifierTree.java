...
public class ClassifierTree implements Drawable, Serializable, RevisionHandler, CapabilitiesHandler {
  ...
  public void buildClassifier(Instances data) throws Exception {
	...
	System.out.println("ClassifierTree # inside buildClassifier(Instances) -- data.numInstances() " + data.numInstances());
...
//	System.out.println("ClassifierTree # inside buildTree(Instances, boolean) ==> " + "keepData = " + keepData);
  ...
  public void buildTree(Instances train, Instances test, boolean keepData)
	...
	System.out.println("ClassifierTree # inside buildTree(Instances, Instances, boolean)");
  ...
  public double classifyInstance(Instance instance) throws Exception {
    ...
    Ontology reference = new Interval(0,1);
    ...
    if (m_objectInt == 0) {
    	for (j = 0; j < instance.numClasses(); j++) {
    		currentProb = getProbs(j, instance, reference, 1);
  ...
  public double[] distributionForInstance(Instance instance,
    ...
    /**
     * TODO
     * Do it better, now it is horrible.
     */
    if (m_objectInt == 0) {
    	for (int i = 0; i < doubles.length; i++) {
//    		System.out.println("\t\tClassifierTree.distributionForInstance(Instance,boolean) # Starting from reference=" + reference + " with classIndex=" + i);
    		doubles[i] = getProbs(i, instance, reference, 1);
    	}
    }
    else {
	    for (int i = 0; i < doubles.length; i++) {
  ...
  protected ClassifierTree getNewTree(Instances data) throws Exception {
	...
	System.out.println("ClassifierTree # inside getNewTree(Instances) -- data.numInstances() " + data.numInstances());
  ...
  protected ClassifierTree getNewTree(Instances train, Instances test)
	...
	System.out.println("ClassifierTree # inside getNewTree(Instances,Instances) -- train.numInstances() " + train.numInstances() + " test.numInstances() " + test.numInstances());
    ClassifierTree newTree = new ClassifierTree(m_toSelectModel);
...
//System.out.println("ClassifierTree.getProbsLaplace");
    double prob = 0;
...
//	  System.out.println("ClassifierTree.getProbs");
   ...
   * @throws Exception if something goes wrong
   */
  private double getProbs(int classIndex, Instance instance, Ontology reference, double weight)
    throws Exception {

//	  System.out.println("ClassifierTree.getProbs with reference");
...
//      System.out.println("\tClassifierTree.getProbs(int, Instance, Ontology, double) # oldReference=" + reference);
      int treeIndex = localModel().whichSubset(instance, reference);
      
      // Getting split ontology (the witness) that has been computed during whichSubset(instance, reference)
      reference = localModel().getSplitOntology();
      
//      System.out.println("\tClassifierTree.getProbs(int, Instance, Ontology, double) # newReference=" + reference + "\n");
      if (treeIndex == -1) {
        double[] weights = localModel().weights(instance);
        for (int i = 0; i < m_sons.length; i++) {
          if (!son(i).m_isEmpty) {
            prob += son(i).getProbs(classIndex, instance, reference, weights[i] * weight);
          }
        }
        return prob;
      } else {
...
//        	System.out.println("calling on " + treeIndex + " son");
          return son(treeIndex).getProbs(classIndex, instance, reference, weight);
        }
      }
    }
  }
...
//	  System.out.println("ClassifierTree.getMembershipValues");
