...
import timeseries.languages.TripleAttributeVirtualMinMax;
...
public abstract class ClassifierSplitModel
  ...
  /**
   * Temporal ontology for classification.
   */
  protected Ontology m_splitOntology;
   ...
   * true 	==> max
   */
  protected boolean m_virtualOrder;
   ...
   * the minimum and maximum for each attribute. <br>
   * 
   * In particular, the key is the juxtaposing of strings:<br>
   * 	1) instance index ranging from 0 to m_allData.numInstances()-1 <br>
   * 	2) ontology representing the world (to go on) <br>
   * 
  ...
  public final double classifyInstance(Instance instance)
    ...
    System.out.println("\t\tClassifierSplitModel.classifyInstance");
  ...
  public Instances [] split(Instances data)
       throws Exception { 
//	  System.out.println("ClassifierSplitModel.split calls whichSubset 2 times");
    ...
    for (Instance instance : data) {
      int subset = whichSubset(instance);
      if (subset > -1) {
        instances[subset].add(instance, false);
      } 
      else {
        double[] weights = weights(instance);
        for (int j = 0; j < m_numSubsets; j++) {
	      if (Utils.gr(weights[j], 0)) {
	        instances[j].add(instance, false);
  ...
  /**
   * TODO
   * da rifare meglio, non è implementato in C45Split
   */
  public abstract int whichSubset(Instance instance, Ontology reference) throws Exception;
  
  public void setSplitOntology(Ontology splitOntology) {
	  m_splitOntology = splitOntology;
  }

  public Ontology getSplitOntology() {
	  return this.m_splitOntology;
  }
  
  public boolean getVirtualOrder() {
	  return m_virtualOrder;
  }
}
